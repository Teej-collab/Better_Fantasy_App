...
### Native app wrapper (Capacitor, Aug 27 2026)

`frontend/ios/` and `frontend/android/` are real Xcode/Gradle projects
generated by `npx cap add ios` / `npx cap add android` — not started
from scratch, so don't hand-edit their generated boilerplate beyond
what Capacitor itself manages.

**This is a remote-URL wrapper, not a bundled app.** The frontend is a
full Next.js SSR app — real server-side session cookies, WebSocket
connections for the draft room/chat/Gamecast — with no static export
configured. Bundling a local copy into the native shell would break
auth and every live feature, so `capacitor.config.ts`'s `server.url`
points the WebView straight at the live production deployment
(`https://weekend-league-web.vercel.app`) instead. `capacitor-www/` is
just a placeholder the native project generators require to exist —
it's never actually shown, and is deliberately separate from the real
Next.js `public/` dir so it can't collide with anything served on the
live site.

**Practical consequence**: a normal web-side change (anything in
`src/`) needs no native rebuild at all — the app just loads the
updated live site next time it opens. A native-level change (icons,
splash screen, native plugins, `capacitor.config.ts` itself) does need
`npx cap sync` plus a real rebuild in Xcode/Android Studio.

**Opening the native projects** (requires a real Mac + Xcode for iOS,
Android Studio for Android — neither exists in this sandboxed session,
so this part is on you):
```
npx cap open ios      # opens ios/App/App.xcworkspace in Xcode
npx cap open android  # opens android/ in Android Studio
```

**Icons/splash screens**: generated from `frontend/assets/logo.png`
(currently 512×512 — Capacitor's ideal source is 1024×1024, so a
higher-res source logo would sharpen these if one becomes available)
via `@capacitor/assets`:
```
npx @capacitor/assets generate --ios --android \
  --iconBackgroundColor '#23212c' --iconBackgroundColorDark '#23212c' \
  --splashBackgroundColor '#23212c' --splashBackgroundColorDark '#23212c'
```
**Important**: always pass `--ios --android` explicitly (never bare
`generate`) — without a platform flag, this tool also regenerates a
`pwa` icon set and **overwrites `frontend/public/manifest.json`** with
broken relative paths, silently corrupting the real site's PWA icons.
Hit this exact bug while first setting this up; reverted via `git
checkout -- frontend/public/manifest.json` before it shipped. If it
ever happens again, that's the fix.

**What's still needed before this can actually go to a store** — none
of it is possible from an unattended coding session, all of it is on
you:
- An Apple Developer Program enrollment ($99/yr) for iOS signing/
  submission, and a Google Play Console account ($25 one-time) for
  Android.
- Real code signing (Xcode-managed signing is easiest to start with)
  and a Play Console signing key.
- Store listing content — screenshots, description, privacy policy
  URL (the app has none yet, and both stores require one), and
  answering both stores' data-safety/privacy questionnaires honestly
  based on what this app actually collects (Discord OAuth identity,
  no payments, no ads/trackers).
- In-app account deletion — both stores require this for any app with
  account creation; today it's a disabled "Soon" button in
  `AccountSection.tsx`.
- A real device/simulator pass once the wrapper is buildable — native
  chrome, safe areas, and OS-level permissions (push notification
  prompts, etc.) behave differently in a real wrapped app than in a
  browser tab, and haven't been checked yet.
