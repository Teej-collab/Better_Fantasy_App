...
async def get_week_power_rankings(conn, season: int, week: int):
    rows = await queries.get_week_power_rankings(conn, season, week)
    return [
        {
            "team_id": r["team_id"],
...
async def get_latest_ranked_week(conn, season: int) -> int | None:
    return await queries.get_latest_ranked_week(conn, season)


async def get_season_trend(conn, season: int):
    rows = await queries.get_season_power_rank_trend(conn, season)
...
async def get_all_time_indices(conn):
    most_at_one = await queries.most_weeks_at_number_one(conn, _TOP_N)
    best_avg_rank = await queries.career_avg_power_rank(conn, _TOP_N, descending=False)
    luckiest = await queries.career_avg_luck(conn, _TOP_N, descending=True)
    unluckiest = await queries.career_avg_luck(conn, _TOP_N, descending=False)
    toughest = await queries.career_avg_sos(conn, _TOP_N, descending=True)
    easiest = await queries.career_avg_sos(conn, _TOP_N, descending=False)
