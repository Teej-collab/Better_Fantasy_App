...
async def test_list_feedback_returns_real_rows_for_a_commissioner(pool, monkeypatch):
    ...
    await _post("/feedback", json={"message": "A real feedback item to list"}, cookies=submitter_cookies)
    ...
    assert any(item["message"] == "A real feedback item to list" for item in items)
